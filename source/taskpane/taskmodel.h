...
public:
    ...
    int taskCount(Id categoryId);
    int errorTaskCount(Id categoryId);
    int warningTaskCount(Id categoryId);
    int unknownTaskCount(Id categoryId);
...
private:
    ...
    QHash<Id,CategoryData> m_categories; // category id to data
...
public:
    ...
    QList<Id> filteredCategories() const { return m_categoryIds; }
    void setFilteredCategories(const QList<Id> &categoryIds) { m_categoryIds = categoryIds; invalidateFilter(); }

    Task task(const QModelIndex &index) const
    { return m_sourceModel->task((index)); }
