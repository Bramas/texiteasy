...
{
    m_categories.insert(0, CategoryData());
}

int TaskModel::taskCount(Id categoryId)
{
    return m_categories.value(categoryId).count;
}

int TaskModel::errorTaskCount(Id categoryId)
{
    return m_categories.value(categoryId).errors;
}

int TaskModel::warningTaskCount(Id categoryId)
{
    return m_categories.value(categoryId).warnings;
}

int TaskModel::unknownTaskCount(Id categoryId)
{
    return m_categories.value(categoryId).count
...
QList<Task> TaskModel::tasks(Id categoryId) const
{
    if (categoryId == 0)
        return m_tasks;
...
void TaskModel::clearTasks(Id categoryId)
{
...
QList<Id> TaskModel::categoryIds() const
{
    QList<Id> categories = m_categories.keys();
    categories.removeAll(0); // remove global category we added for bookkeeping
    return categories;
}

QString TaskModel::categoryDisplayName(Id categoryId) const
{
...
void TaskModel::setFileNotFound(const QModelIndex &idx, bool b)
{
    if (idx.isValid() && idx.row() < m_tasks.count()) {
        m_fileNotFound.insert(QDir::toNativeSeparators(m_tasks[idx.row()].file), b);
